...
The hub assigns opaque conversation IDs; source connection plus external occurrence ID is unique. Segments are immutable and have stable citation IDs, adapter deduplication keys, relative speech timestamps when available, and receipt timestamps. Thread attachments have independent explicit acknowledgement cursors. Imports create separate conversations; reconciling cleaned-up transcript versions is deferred without overwriting prior citations.

## Vertical slice
Import UTF-8 WebVTT, SRT, or plain text (maximum 1 MB), view/search the transcript, attach a BB thread, retrieve relevant passages with IDs and timing through tools/CLI, and acknowledge a reading cursor. State survives plugin reload. Plain text retains paragraphs and optional speaker prefixes with null timestamps. Malformed timed files fail atomically. Limit each segment to 2,000 characters and retrieval pages to 20 by default, 30 maximum. Larger import passages may split preserving attribution and timing.
